...
export class IA {
  ...
  evaluate (board, players, isIA, finished) {
    ...
    let result = 100;
    if (!finished) {
      console.log("Not finished", this.evaluatedMoves, result);
    } else {
      result = (isIA ? 1 : -1) * (WEIGHT - players[isIA ? 1 : 0].pieces.length);
      console.log("finished", this.evaluatedMoves, result);
    }
